...
|--------|-------|-------------|
| `subscribe(handler)` | — | Register callback |
| `emit(tick)` | yes | Dispatch one tick |
| `listen()` | yes | WebSocket → queue |
| `process()` | yes | queue → emit |
| `start_sentinel()` | — | Background listen+process |
| `ingest(payload)` | yes | Enqueue replay tick |
| `stop()` | yes | Cancel tasks |
...
| Method | Description |
|--------|-------------|
| `run_replay(ticks: list[dict])` | Async demo pipeline |
| `run_live()` | WebSocket until cancelled |
| `shutdown()` | Stop audit + supervisor |
